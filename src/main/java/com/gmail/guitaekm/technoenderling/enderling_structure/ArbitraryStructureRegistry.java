...
public class ArbitraryStructureRegistry implements SimpleSynchronousResourceReloadListener, ServerLifecycleEvents.ServerStarting {
    ...
    public Identifier getFabricId() {
        return new Identifier(TechnoEnderling.MOD_ID, "arbitrary_structures");
    }
    ...
    public void reload(ResourceManager manager) {
        ...
        for(Identifier id : manager.findResources("structures", path -> path.endsWith(".nbt"))) {
            ...
            try {
                ...
                TechnoEnderling.LOGGER.error("Error with file opening of enderling structure json " + id.toString(), e);
                throw new RuntimeException("handle exception while loading data pack");
            } catch(Exception e) {
                TechnoEnderling.LOGGER.error("Error occurred while loading resource arbitrary structure nbt " + id.toString(), e);
            }
        }
    }
